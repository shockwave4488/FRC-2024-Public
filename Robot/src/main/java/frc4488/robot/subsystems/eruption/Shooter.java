...
import frc4488.lib.controlsystems.SetPointProfile;
import frc4488.lib.devices.LinearServo;
...
public class Shooter extends ShockwaveSubsystemBase {
  ...
  public final DoneCycleMachine<NumberCondition> flywheelVelocityMachine;
  ...
  public Shooter(
    ...
    flywheelVelocityMachine =
        new DoneCycleMachine<>(
            new NumberCondition(
                () ->
                    masterFlywheel.getSelectedSensorVelocity()
                        * 600
                        * PULLEY_RATIO
                        / ENCODER_TICKS_PER_ROTATION));
    flywheelVelocityMachine.withName("Flywheel velocity machine");
    flywheelVelocityMachine.getCondition().deviceControlCallback.set(rpm -> targetRPM = rpm);
    flywheelVelocityMachine.getCondition().doneRange.set(ERROR_EPSILON);
    flywheelVelocityMachine.minDoneCycles.set(MIN_COUNT_CYCLES);
    LeveledSmartDashboard.INFO.putData(flywheelVelocityMachine);
  ...
  */
  ...
  public void setRPM(double rpm) {
    flywheelVelocityMachine.getCondition().setDesiredValue(rpm);
  }
  ...
  public boolean isReady() {
    return flywheelVelocityMachine.getStatus().isReady() && hoodReady();
  }
  ...
  public void logShooterValues(double yOffset, double RPM, double hoodLevel) {
    logger
        ...
        .println(
            LogLevel.INFO,
            "Shot Log: yOffset: " + yOffset + "  RPM: " + RPM + "  hoodLevel: " + hoodLevel);
  }
  ...
  public void updateSmartDashboard() {
    LeveledSmartDashboard.INFO.putNumber("Shooter Target Speed", targetRPM);
    LeveledSmartDashboard.INFO.putNumber(
        "Shooter Actual Speed", flywheelVelocityMachine.getCondition().getActualValue());
    LeveledSmartDashboard.INFO.putNumber("Desired Shooter Hood Position", desiredHoodPosition);
  ...
  public void setUpTrackables(LogManager logger) {
    logger
        .getLogFile("Shooter/FlywheelDesiredRPM")
        .addTracker("ShooterFlywheelDesiredRPM", () -> targetRPM, 5);

    logger
        .getLogFile("Shooter/FlywheelActualRPM")
        .addTracker(
            "ShooterFlywheelActualRPM", flywheelVelocityMachine.getCondition()::getActualValue, 5);

    logger
        .getLogFile("Shooter/DesiredHoodPosition")
        .addTracker("ShooterDesiredHoodPosition", () -> desiredHoodPosition, 5);

    logger.getLogFile("Shooter/IsReady").addTracker("ShooterIsReady", this::isReady, 5);
  }

  @Override
  public void onSStop(boolean robotEnabled) {
    masterFlywheel.set(ControlMode.Disabled, 0);
  }
