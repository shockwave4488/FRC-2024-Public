...
/**
 * DFT stands for Discrete Fourier Transform and is the most widely used Fourier Transform. You will
 * never want to use this class due to the fact that it is a brute force implementation of the DFT
 * and as such is quite slow. Use an FFT instead. This exists primarily as a way to ensure that
 * other implementations of the DFT are working properly. This implementation expects an even <code>
 * timeSize</code> and will throw and IllegalArgumentException if this is not the case.
 *
...
public class DFT extends FourierTransform {
  /**
   * Constructs a DFT that expects audio buffers of length <code>timeSize</code> that have been
   * recorded with a sample rate of <code>sampleRate</code>. Will throw an IllegalArgumentException
